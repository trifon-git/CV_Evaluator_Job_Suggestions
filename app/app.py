    ...
    if cv_text:
        ...
        with st.spinner('🧠 Analyzing CV content & searching database... Please wait...'):
            try:
                ...
                if method_used and method_used.startswith("Error"):
                    st.error(f"⚠️ **Search Failed:** {method_used}")
                    st.error("Could not retrieve job matches.")
                    # Add more detailed error information
                    st.error("ChromaDB Connection Details:")
                    st.code(f"ChromaDB Host: {chroma_host}")
                    st.error("Please check if ChromaDB is running and accessible.")
                 ...
                 st.error(f"Error details: {str(find_err)}")
