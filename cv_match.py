...
TOP_N_RESULTS = 10
CV_FILE_PATH = "cv_text_example.md"
...
def find_similar_jobs(db_path, model, cv_text, top_n=10):
    ...
    try:
        ...
        if use_vss:
            ...
            query = """
                SELECT
                    j.rowid AS job_rowid, j.Title, c.Company, a.Area, j.JobIndexURL,
                    v.distance
                FROM vss_jobs v
            ...
            query = """
                SELECT
                    j.rowid AS job_rowid, j.Title, c.Company, a.Area, j.JobIndexURL, j.embedding
                FROM jobs j
                LEFT JOIN companies c ON j.Company_ID = c.id
                LEFT JOIN areas a ON j.Area_ID = a.id
...
def main():
    ...
    for i, job in enumerate(matches):
        ...
        print(f"   URL: {job.get('JobIndexURL', '#')}")
        print()
