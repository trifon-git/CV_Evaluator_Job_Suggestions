...
elif NGROK_API_URL_FROM_ENV: print(f"test_llm.py: [STEP3]llm_skill_extractor.py will use NGROK API: {NGROK_API_URL_FROM_ENV}", flush=True)
...
if MONGO_URI_FROM_ENV and MONGO_DB_NAME_FROM_ENV and MONGO_COLLECTION_FROM_ENV:
    ...
    try:
        ...
        mongo_samples = list(collection.find(
            {"html_content": {"$exists": True, "$ne": None, "$ne": ""}, "Status": "active"},
            {"_id": 1, "Title": 1, "html_content": 1}
        ).limit(30)) # Fetch 30 samples for testing
        if mongo_samples:
            print(f"test_llm.py: Found {len(mongo_samples)} active samples from MongoDB.", flush=True)
            for i, sample_doc in enumerate(mongo_samples):
                doc_id_str = str(sample_doc.get('_id'))
                ...
                test_llm_with_chunking(html_content, 
        ...
        else: print("test_llm.py: No suitable active samples found in MongoDB.", flush=True)
